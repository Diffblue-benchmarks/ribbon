...
public class RibbonResourceFactoryDiffblueTest {
  ...
  public void testGetTransportFactory() {
    ...
    // Assert
    assertTrue(actualTransportFactory instanceof RibbonTransportFactory.DefaultRibbonTransportFactory);
   ...
   */
  @Test
  public void testGetClientConfigFactory2() {
    // Arrange
    ClientConfigFactory clientConfigFactory = mock(ClientConfigFactory.class);
    DefaultClientConfigImpl emptyConfig = DefaultClientConfigImpl.getEmptyConfig();
    when(clientConfigFactory.newConfig()).thenReturn(emptyConfig);

    // Act
    IClientConfig actualNewConfigResult = (new DefaultResourceFactory(clientConfigFactory,
        new RibbonTransportFactory.DefaultRibbonTransportFactory(mock(ClientConfigFactory.class))))
            .getClientConfigFactory()
            .newConfig();

    // Assert
    verify(clientConfigFactory).newConfig();
    assertSame(emptyConfig, actualNewConfigResult);
  }
}
